...
export const getVerificationTokenCheckedByToken = async (token: string)=>{
    try{
        const verificationToken = await db.query.emailTokens.findFirst({
...
export const generateEmailVerifiactionToken = async(email: string)=>{
...
export const newVerification = async (token: string)=>{
    const existingToken = await getVerificationTokenCheckedByToken(token);
    if(!existingToken) return {error:"token not found"}
    const hasExpired = new Date(existingToken.expires)<new Date()

    if (hasExpired) return {error:"il tooken per registrarsi è scaduto registrati nuovamente"}
